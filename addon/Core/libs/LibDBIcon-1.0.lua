...
local function updatePosition(button)
	...
	local raio = 77 --80 Modifiquei para ajustar MoRBiDuS
	if quadTable[q] then
		x, y = x*raio, y*raio
	else
		local diagRadius = math.sqrt(2*(raio)^2)-10
		x = math.max(-raio, math.min(x*diagRadius, raio))
		y = math.max(-raio, math.min(y*diagRadius, raio))
	end
...
local function onDragStop(self)
	...
	self.isMoving = nil
	if self.dataObject.OnDragStop then
		self.dataObject.OnDragStop(self)
	end	
end
