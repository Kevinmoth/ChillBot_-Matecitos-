...
local udk_gui = {
	settingsfile = "A1 Unholy Death Knight PvE 1.1.json",
	{ type = "title", text = "|cffdc0a17Unholy Death Knight PvE |cffcece0cA1 v1.1" },
	{ type = "separator" },
	{ type = "entry", text = "\124T"..udk_icons.Chains_of_Ice..":22:22\124t |cffdc0a17Ледяные оковы", enabled = false, key = "gui_Chains_of_Ice" },
	{ type = "entry", text = "\124T"..udk_icons.Pestilence..":22:22\124t |cffdc0a17Мор", enabled = false, value = 3, min = 2, max = 10, step = 1, width = 30, key = "gui_Pestilence" },
	{ type = "entry", text = "\124T"..udk_icons.Mind_Freeze..":22:22\124t |cffdc0a17Заморозка разума", enabled = false, key = "gui_Mind_Freeze" },
	{ type = "entry", text = "|cffdc0a17Тринкеты", enabled = false, key = "gui_Trinkets" },
    { type = "separator" },
	{ type = "entry", text = "|cffdc0a17Лог", enabled = false, key = "gui_debug" },
}
...
local function gui_load()
	ni.GUI.AddFrame("A1 Unholy DK PvE", udk_gui)
end
local function gui_unload()
	ni.GUI.DestroyFrame("A1 Unholy DK PvE")
end
local guid_enemies = {}
...
local udk_functions = {
    ...
    ["Combat Pause"] = function ()
        ...
        or (ni.unit.hp("target") == 100 and UnitInRaid("player")) then
			return true
        end
    end,
	["Cache"] = function()
	...
	["Scourge Strike"] = function()
        ...
        --and ni.unit.debuff("target", udk_skills.Blood_Plague, "player")
        then
    ...
    ["Horn of Winter"] = function()
        ...
        or ni.player.buffremaining(udk_skills.Horn_of_Winter) <= 60) then
	...
	["Raise Dead"] = function()
        if ni.player.hasitem(37201)
        and ni.player.itemcd(37201) == 0 then
...
ni.bootstrap.profile("A1 Unholy Death Knight PvE", udk_rotation, udk_functions, gui_load, gui_unload)