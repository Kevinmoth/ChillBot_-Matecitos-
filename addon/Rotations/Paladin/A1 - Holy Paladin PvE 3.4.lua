...
local hpal_gui = {
	settingsfile = "A1 - Holy Paladin PvE3-4.json",
	{ type = "title", text = "|cfff58cbaHoly Paladin PvE |cffcece0cA1 v3.4" },
...
local function gui_load()
	ni.GUI.AddFrame("A1 Holy PvE3-4", hpal_gui)
end
local function gui_unload()
	ni.GUI.DestroyFrame("A1 Holy PvE3-4")
end
    ...
    ["Lay on Hands"] = function()
        ...
        and UnitAffectingCombat("player") then
            for _, member in ipairs(ni.members) do
                ...
                if not ni.unit.debuff(member.unit, hpal_skills.Forbearance)
                and member.hp < 20
...
ni.bootstrap.profile("A1 - Holy Paladin PvE 3.4", hpal_rotation, hpal_abilities, gui_load, gui_unload)