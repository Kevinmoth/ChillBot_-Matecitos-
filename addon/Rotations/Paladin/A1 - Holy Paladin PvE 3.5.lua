...
local hpal_gui = {
	settingsfile = "A1 - Holy Paladin PvE3-5.json",
	{ type = "title", text = "|cfff58cbaHoly Paladin PvE |cffcece0cA1 v3.5" },
...
local function gui_load()
	ni.GUI.AddFrame("A1 Holy PvE3-5", hpal_gui)
end
local function gui_unload()
	ni.GUI.DestroyFrame("A1 Holy PvE3-5")
end
...
local hpal_abilities = {
        ...
        and UnitAffectingCombat("player") then
            for _, z in ipairs(OverHealing) do
                for _, x in ipairs(CriticalDebuff) do
                    ...
                    if ((ni.unit.hpraw(ni.members[1].unit) >= HolyLightHealing*X or ((ni.unit.debuff(ni.members[1].unit, z) and member.hp <= 90)
                    or ni.unit.debuff(ni.members[1].unit, x)))
                    and ni.spell.valid(ni.members[1].unit, hpal_skills.Holy_Light, false, true, true)) then
                        return ni.spell.cast(hpal_skills.Holy_Light, ni.members[1].unit)
                    end
                end
    ...
    ["Flash of Light"] = function ()
        ...
        and UnitAffectingCombat("player") then
                ...
                and ni.unit.hpraw(ni.members[1].unit) >= FlashofLightHealing*X then
    ...
    ["Lay on Hands"] = function()
        ...
        and UnitAffectingCombat("player") then
                ...
                if not ni.unit.debuff(ni.members[1].unit, hpal_skills.Forbearance)
    ...
    ["Sacred Shield"] = function()
        if ni.spell.available(hpal_skills.Sacred_Shield) then
            if ni.unit.exists("focus")
            and UnitAffectingCombat("focus") then
                if not ni.unit.buff("focus", hpal_skills.Sacred_Shield)
            ...
            and UnitAffectingCombat("player") then
                if not ni.unit.buff("player", hpal_skills.Sacred_Shield) then
    ...
    ["Hand of Salvation"] = function()
        if UnitAffectingCombat("player") then
            for _, member in ipairs(ni.members) do
                if ni.unit.threat(member.unit) > 1
    ...
    ["Holy Light (AOE)"] = function()
        ...
        and not ni.unit.ismoving("player") then
                ...
                if #ni.members.inrangebelowraw(ni.members[1].unit, 8, HolyLightHealingAOE) >= 4
                and ni.spell.valid(ni.members[1].unit, hpal_skills.Holy_Light, false, true, true) then
                    return ni.spell.cast(hpal_skills.Holy_Light, ni.members[1].unit)
                end
        end
...
ni.bootstrap.profile("A1 - Holy Paladin PvE 3.5", hpal_rotation, hpal_abilities, gui_load, gui_unload)