...
local pvpdispell = {10308, 45524, 45524, 53313, 64695, 4167, 64804, 42917}
...
local abilities = {
    ...
    ["LightOfHeaven"] = function()
		Enemies = ni.unit.enemiesinrange("player", 12);
		if ni.spell.available(spell.LightOfHeaven)
		and not ni.unit.ismoving("player")
		and #Enemies == 0 then
			for i = 1, #ni.members do
				if ni.unit.hpraw(ni.members[i].unit) >= HolyLightHealing
				and ni.spell.valid(ni.members[i].unit, spell.LightOfHeaven, false, true, true) then
    ...
    ["HolyShock"] = function()
		if ni.spell.available(spell.HolyShock) then
			for i = 1, #ni.members do
				if ni.unit.hpraw(ni.members[i].unit) >= HolyShockHealing*1.6
    ...
    ["HammerOfJustice"] = function()
		...
		for	i = 1, #Enemies do
			...
			local interchan = select(8, UnitChannelInfo(tar));
			if ni.spell.available(spell.HammerOfJustice)
			and ((ni.unit.iscasting(tar) and intercast) or (ni.unit.ischanneling(tar) and interchan) or ni.unit.hp(tar) <= 20)
    ...
    ["ParticleOfLight"] = function()
        if (not ni.player.buff(spell.ParticleOfLight) or ni.player.buffremaining(spell.ParticleOfLight) <= 5)
    ...
    ["Judgement"] = function()
        if ni.spell.available(53407)
        and ni.spell.valid("target", 53407, true, true) then
        	ni.spell.cast(53407, "target")
        end
    end,
...
ni.bootstrap.profile("Holy_broken_pvp", queue, abilities, OnLoad, OnUnLoad);