...
local hpal_gui = {
	settingsfile = "A1 - Holy Paladin PvE4-0.json",
	{ type = "title", text = "|cfff58cbaHoly Paladin PvE |cffcece0cA1 v4.0" },
...
local function gui_load()
	ni.GUI.AddFrame("A1 Holy PvE4-0", hpal_gui)
end
local function gui_unload()
	ni.GUI.DestroyFrame("A1 Holy PvE4-0")
end
    ...
    ["Flash of Light"] = function ()
        ...
        and UnitAffectingCombat("player") then
            for _, member in ipairs(ni.members) do
                ...
                if ni.spell.valid(member.unit, hpal_skills.Flash_of_Light, false, true, true)
                and ni.unit.hp(member.unit) < 88 then
                    return ni.spell.cast(hpal_skills.Flash_of_Light, member.unit)
                end
            end
        end
        ...
        if enabled then
            if ni.spell.available(hpal_skills.Cleanse) then
                for _, member in ipairs(ni.members) do
                    ...
                    and ni.spell.valid(member.unit, hpal_skills.Cleanse, false, true, true) then
                        return ni.spell.delaycast(hpal_skills.Cleanse, member.unit, 1.3)
                    end
                end
            end
        end
...
ni.bootstrap.profile("A1 - Holy Paladin PvE 4.0", hpal_rotation, hpal_abilities, gui_load, gui_unload)