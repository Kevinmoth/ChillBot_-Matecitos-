...
local hpal_gui = {
	settingsfile = "A1 - Holy Paladin PvE4-2.json",
	{ type = "title", text = "|cfff58cbaHoly Paladin PvE |cffcece0cA1 v4.2" },
	{ type = "separator" },
...
local function gui_load()
	ni.GUI.AddFrame("A1 Holy PvE4-2", hpal_gui);
end
local function gui_unload()
	ni.GUI.DestroyFrame("A1 Holy PvE4-2");
end
local CriticalCast = {71224, "Сожжение", "Ледяное дыхание", "Огненное дыхание", "Священный огонь", "Дыхание Пустоты", "Пожирающее пламя", "Ледяные шипы", "Статический сбой", "Чародейские стрелы"}
...
local NoHeal = {30843, 41292, 45996, 55593}
...
local hpal_abilities = {
                ...
                for j in pairs(CloseEnemies_guid2) do
                    ...
                    if ni.spell.valid(UndeadOrDemon, hpal_skills.Fear, false, true, false) then
                        ...
                        and not ni.unit.debuff(UndeadOrDemon, hpal_skills.Fear) then
                            LastFearing = GetTime()
                            ni.spell.cast(hpal_skills.Fear, UndeadOrDemon)
                            return true;
                        end
                    end
                end
    ...
    ["Seal of Wisdom"] = function ()
        ...
        and ni.spell.available(hpal_skills.Seal_of_Wisdom) then
            ni.spell.cast(hpal_skills.Seal_of_Wisdom)
            return true;
        end
    end,
    ["Divine Plea"] = function ()
        ...
        and ni.spell.available(hpal_skills.Divine_Plea) then
            ni.spell.cast(hpal_skills.Divine_Plea)
            return true;
        end
    end,
    ["Divine Illumination"] = function ()
        ...
        and ni.spell.cd(hpal_skills.Divine_Illumination) == 0 then
            ni.spell.cast(hpal_skills.Divine_Illumination)
            return true;
        end
    end,
    ["Divine Favor"] = function ()
        ...
        and UnitAffectingCombat("player") then
            if ni.members.below(75) >= 9 then
                ni.spell.cast(hpal_skills.Divine_Favor)
                return true;
            end
        end
    end,
        ...
        if enabled then
            if ni.spell.available(hpal_skills.Cleanse) then
                for i = 1, #ni.members do
                    ...
                    if (ni.unit.debuff(Ally, raid_debuffs.Necrotic_Plague)
                    and ni.unit.debuffremaining(Ally, raid_debuffs.Necrotic_Plague) < 12
                    and #ni.unit.enemiesinrange(Ally, 4) >= 1)
                    --or (ni.unit.debuff(Ally, raid_debuffs.Fiery_Combustion)
                    --and ni.unit.debuffremaining(Ally, raid_debuffs.Fiery_Combustion) < 24)
                    --or (ni.unit.debuff(Ally, raid_debuffs.Soul_Consumption)
                    --and ni.unit.debuffremaining(Ally, raid_debuffs.Soul_Consumption) < 24)
                    and ni.spell.valid(Ally, hpal_skills.Cleanse, false, true, true) then
                        ni.spell.cast(hpal_skills.Cleanse, Ally)
                        return true;
                    end
                end
            end
    ...
    ["Holy Light no combat"] = function ()
        ...
        and not ni.player.ismoving() then
            ...
            and ni.spell.valid(ni.members[1].unit, hpal_skills.Holy_Light, false, true, true) then
                ni.spell.cast(hpal_skills.Holy_Light, ni.members[1].unit)
                return true;
            end
        end
    end,
    ["T5 Stack"] = function ()
        ...
        and ni.spell.available(hpal_skills.Flash_of_Light) then
            ni.spell.cast(hpal_skills.Flash_of_Light, "player")
            return true;
        end
    end,
            ...
            for _, z in ipairs(OverHealing) do
                for _, x in ipairs(CriticalDebuff) do
                    for i = 1, #ni.members do
                        ...
                        if ((ni.unit.debuff(Ally, z) and AllyHP <= 90)
                        or ni.unit.debuff(Ally, x))
                        and ni.spell.valid(Ally, hpal_skills.Holy_Light, false, true, true) then
                            ni.spell.cast(hpal_skills.Holy_Light, Ally)
                            return true;
                        end
                    end
                end
    ...
    ["Holy Light"] = function ()
            if not ni.unit.debuff(ni.members[1].unit, raid_debuffs.Enfeeble) then
                if ni.spell.available(hpal_skills.Holy_Light)
                and not ni.player.ismoving()
                and UnitAffectingCombat("player") then
                    if ni.unit.hpraw(ni.members[1].unit) >= HolyLightHealing
                    and ni.spell.valid(ni.members[1].unit, hpal_skills.Holy_Light, false, true, true) then
                        ni.spell.cast(hpal_skills.Holy_Light, ni.members[1].unit)
                        return true;
                    end
    ...
    ["Holy Shock"] = function ()
            if not ni.unit.debuff(ni.members[1].unit, raid_debuffs.Enfeeble) then
                ...
                and UnitAffectingCombat("player") then
                    if ni.unit.hpraw(ni.members[1].unit) >= HolyShockHealing
                    ...
                    and ni.spell.valid(ni.members[1].unit, hpal_skills.Holy_Shock, false, true, true) then
                        ni.spell.cast(hpal_skills.Holy_Shock, ni.members[1].unit)
                        return true;
                    end
                end
            end
    ...
    ["Flash of Light"] = function ()
            if not ni.unit.debuff(ni.members[1].unit, raid_debuffs.Enfeeble) then
                ...
                and (not ni.unit.ismoving("player") or ni.player.buff(hpal_skills.Infusion_of_Light))
                and UnitAffectingCombat("player") then
                    if ni.spell.valid(ni.members[1].unit, hpal_skills.Flash_of_Light, false, true, true)
                    and ni.unit.hpraw(ni.members[1].unit) >= FlashofLightHealing then
                        ni.spell.cast(hpal_skills.Flash_of_Light, ni.members[1].unit)
                        return true;
                    end
                end
            end
    ...
    ["Lay on Hands"] = function()
            ...
            and not ni.unit.debuff(ni.members[1].unit, hpal_skills.Forbearance) then
                if ni.spell.available(hpal_skills.Lay_on_Hands)
                and UnitAffectingCombat("player") then
                    if ni.members[1].hp < 20
                    and ni.spell.valid(ni.members[1].unit, hpal_skills.Lay_on_Hands, false, true, true) then
                        ni.spell.cast(hpal_skills.Lay_on_Hands, ni.members[1].unit)
                        return true;
                    end
                end
            end
        ...
        if ni.spell.available(hpal_skills.Hammer_of_Justice) then
            for	j in pairs(CloseEnemies_guid) do
                ...
                and ni.spell.valid(caster, hpal_skills.Hammer_of_Justice, true, true) then
                    ni.spell.cast(hpal_skills.Hammer_of_Justice, caster)
                    return true;
                end
            end
        end
    ...
    ["Beacon of Light"] = function()
        ...
        and UnitAffectingCombat("focus") then
            ...
            and ni.spell.valid("focus", hpal_skills.Beacon_of_Light, false, true, true) then
                ni.spell.cast(hpal_skills.Beacon_of_Light, "focus")
                return true;
            end
                ...
                and ni.unit.buff("focus", hpal_skills.Beacon_of_Light, "player") then
                    ...
                    and not ni.player.ismoving() then
                        ni.spell.cast(hpal_skills.Holy_Light, "player")
                        return true;
                    end
                    if not ni.player.buff(hpal_skills.TwoT5)
                    and ni.unit.hpraw("focus") >= HolyShockHealing
                    and ni.spell.available(hpal_skills.Holy_Shock)
                    and ni.spell.valid("focus", hpal_skills.Beacon_of_Light, false, false, true) then
                        ni.spell.cast(hpal_skills.Holy_Shock, "player")
                        return true;
                    end
                end
        end
        ...
        if ni.spell.available(hpal_skills.Sacred_Shield) then
            ...
            and UnitAffectingCombat("focus") then
                ...
                and ni.spell.valid("focus", hpal_skills.Sacred_Shield, false, true, true) then
				    ni.spell.cast(hpal_skills.Sacred_Shield, "focus")
                    return true;
                end
			end
            ...
            and UnitAffectingCombat("player") then
                if not ni.unit.buff("player", hpal_skills.Sacred_Shield, "player") then
				    ni.spell.cast(hpal_skills.Sacred_Shield, "player")
                    return true;
                end
			end
        end
            ...
            if ni.spell.available(hpal_skills.Cleanse) then
                for i = 1, #ni.members do
                    ...
                    and ni.spell.valid(ni.members[i].unit, hpal_skills.Cleanse, false, true, true) then
                        ni.spell.delaycast(hpal_skills.Cleanse, ni.members[i].unit, 1.3)
                        return true;
                    end
                end
            end
    ...
    ["Avenging Wrath"] = function()
		...
		and ni.spell.available(hpal_skills.Avenging_Wrath) then
			ni.spell.cast(hpal_skills.Avenging_Wrath)
            return true;
		end
    end,
    ["Trinkets"] = function()
        ...
        and ni.spell.available(hpal_skills.Holy_Wrath) then
            for	j in pairs(CloseEnemies_guid) do
                ...
                and ni.unit.iscasting(UndeadOrDemon) then
                    ni.spell.cast(hpal_skills.Holy_Wrath)
                    return true;
                end
            end
        end
    ...
    ["Hand of Salvation"] = function()
        if UnitAffectingCombat("player") then
            ...
            and ni.spell.valid(ni.members[1].unit, hpal_skills.Hand_of_Salvation, false, true, true) then
                ni.spell.cast(hpal_skills.Hand_of_Salvation, ni.members[1].unit)
                return true;
            end
        end
    end,
    ["Judgement"] = function()
        ...
        and ni.unit.debuffremaining("target", hpal_skills.Svet, "player") <= 5 then
            ni.spell.cast((hpal_skills.Svet), "target")
            return true;
        end
    end,
        ...
        if CurrentBlessing == 0 then
            ...
            for _, member in ipairs(ni.members) do
                ...
                and ni.spell.valid(member.unit, CurrentBlessing, false, true, true) then
                    ni.spell.cast(CurrentBlessing, member.unit)
                    return true;
                end
            end
        end
    ...
    ["Divine Shield"] = function()
        ...
        and UnitAffectingCombat("player") then
            if not ni.unit.debuff("player", hpal_skills.Forbearance)
            and IsLeftControlKeyDown()
            and ni.spell.available(hpal_skills.Divine_Shield) then
                 ni.spell.cast(hpal_skills.Divine_Shield)
                return true;
            end
        end
    end,
    ["Divine Sacrifice"] = function()
        if (ni.player.buff(hpal_skills.Divine_Shield) or ni.player.buff(hpal_skills.Divine_Protection))
        and ni.spell.available(hpal_skills.Divine_Sacrifice) then
            ni.spell.cast(hpal_skills.Divine_Sacrifice)
            return true;
        end
    end,
    ["Divine Protection"] = function()
        if not ni.player.debuff(raid_debuffs.Enfeeble)
        and ni.spell.cd(hpal_skills.Divine_Shield) ~= 0
        and UnitAffectingCombat("player") then
            if not ni.unit.debuff("player", hpal_skills.Forbearance)
            and IsLeftControlKeyDown()
            and ni.spell.available(hpal_skills.Divine_Protection) then
                ni.spell.cast(hpal_skills.Divine_Protection)
                return true;
            end
        end
    end,
        ...
        if ni.spell.available(hpal_skills.Hand_of_Freedom) then
            for _, x in ipairs(freedomdebuff) do
                for i = 1, #ni.members do
                    ...
                    and Tank(Ally)
                    and ni.spell.valid(Ally, hpal_skills.Hand_of_Freedom, false, true, true)
                    and ni.unit.ismoving(Ally) then
                        ni.spell.cast(hpal_skills.Hand_of_Freedom, Ally)
                        return true;
                    end
                end
            end
	...
	["Righteous Fury Check"] = function()
        if ni.player.aura(hpal_skills.Righteous_Fury) then
            CancelUnitBuff("player", hpal_skills.Righteous_Fury)
            return true;
        end
        if ni.player.aura(hpal_skills.Divine_Sacrifice) then
            CancelUnitBuff("player", hpal_skills.Divine_Sacrifice)
            return true;
        end
	end,
    ["Holy Shock (AOE)"] = function()
        ...
        and ni.player.buff(hpal_skills.TwoT5) then
                for i = 1, #ni.members do
                    local Ally = ni.members[i].unit
                    if not ni.unit.debuff(Ally, raid_debuffs.Enfeeble)
                    and #ni.members.inrangebelow(Ally, 30, 70) >= 5
                    and ni.spell.valid(Ally, hpal_skills.Holy_Shock, false, true, true) then
                        ni.spell.cast(hpal_skills.Holy_Shock, Ally)
                        return true;
                    end
                end
        end
    ...
    ["Holy Light (AOE)"] = function()
        ...
        and not ni.unit.ismoving("player") then
                for i = 1, #ni.members do
                    local Ally = ni.members[i].unit
                    if not ni.unit.debuff(Ally, raid_debuffs.Enfeeble)
                    and #ni.members.inrangebelowraw(Ally, 8, HolyLightHealingAOE) >= 4
                    and ni.spell.valid(Ally, hpal_skills.Holy_Light, false, true, true) then
                        ni.spell.cast(hpal_skills.Holy_Light, Ally)
                        return true;
                    end
    ...
    ["Holy Light (Target)"] = function()
        ...
        and not ni.unit.ismoving("player") then
            ni.spell.cast(hpal_skills.Holy_Light, "target")
            return true;
        end
    end,
        ...
        and ni.unit.exists("targettarget") then
            for _, x in ipairs(CriticalCast) do
                ...
                and cast == x then
                    ...
                    and not ni.unit.ismoving("player") then
                        ni.spell.cast(hpal_skills.Holy_Light, "targettarget")
                        return true;
                    end
                end
            end
            ...
            for _, x in ipairs(CriticalCast) do
                if cast == x then
                    ...
                    and ni.spell.valid("targettarget", hpal_skills.Racial, false, true, true) then
                        ni.spell.cast(hpal_skills.Racial, "targettarget")
                        return true;
                    end
                end
            end
        ...
        if dpsison then
            if not ni.player.isdisarmed() then
                ...
                and ni.spell.valid("target", hpal_skills.ShieldOfRighteousness, true, true, false) then
                    ni.spell.cast(hpal_skills.ShieldOfRighteousness)
                    return true;
                end
            end
        end
    ...
    ["Consecration"] = function ()
        ...
        if dpsison then
            ...
            and ni.spell.valid("target", hpal_skills.ShieldOfRighteousness, false, true, false) then
                ni.spell.cast(hpal_skills.Consecration)
                return true;
            end
        end
    end,
}
ni.bootstrap.profile("A1 - Holy Paladin PvE 4.2", hpal_rotation, hpal_abilities, gui_load, gui_unload)